...
6. **Family Contributor Onboarding Protocol**: When prompted to onboard a child or family member (e.g. Bennett, Brighton, Aria, Benjamin), the agent must assist them in signing up for a GitHub account (via github.com/signup using their Google/family email if needed), configure their Git name and email, authenticate via `gh auth login`, notify Dad to add their new GitHub handle as a collaborator to `Swensation/swentonelli`, verify/prompt for `.env.local` Gemini keys, run `npm test`, and verify the local development server runs without error.
